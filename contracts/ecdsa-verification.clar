;; ECDSA Signature Verification Schema
;; A robust Clarity smart contract for ECDSA signature verification and validation
;; Built on Stacks blockchain for secure cryptographic operations

;; Error codes for ECDSA Verification
(define-constant ERR-INVALID-SIGNATURE (err u100))
(define-constant ERR-INVALID-PUBLIC-KEY (err u101))
(define-constant ERR-INVALID-HASH (err u102))
(define-constant ERR-SIGNATURE-VERIFICATION-FAILED (err u103))
(define-constant ERR-UNSUPPORTED-CURVE (err u104))
(define-constant ERR-INVALID-SIGNATURE-LENGTH (err u105))
(define-constant ERR-SIGNATURE-ALREADY-USED (err u106))
...
(define-constant ERR-TASK-NOT-FOUND (err u112))
(define-constant ERR-USER-NOT-FOUND (err u113))
(define-constant ERR-MILESTONE-NOT-FOUND (err u114))
(define-constant ERR-INVALID-TASK-ASSIGNMENT (err u115))
(define-constant ERR-INVALID-TASK-STATUS (err u116))
(define-constant ERR-INVALID-TASK-DEPENDENCIES (err u117))
(define-constant ERR-INVALID-TASK-ASSIGNEE (err u118))
(define-constant ERR-INVALID-TASK-DEADLINE (err u119))
